...
原设计以老师提供的 `.dat` 为核心。2026-09-10 的当前交付决定改用固定提交、
固定哈希的公开 PCAP，并将传输层明文负载确定性导出为 `.dat`；synthetic 数据仍
只用于工程测试/机制验证，不作为公开数据基准。详见
...
正式实验数据必须来自可审计的老师或固定公开数据。当前公开数据的来源、提交、
许可证、大小和哈希均已记录；若以后收到老师数据，仍按以下 metadata 规则接入：
- 文件大小与 SHA-256；
- 收到日期/版本；
- redistribution 权限；
