...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Execute the synthetic LLM-only mechanism baseline without "
            "teacher-data or real-LLM benchmark claims."
        )
    )
    ...
    execution = run_llm_only_mechanism_baseline(
        args.outdir / "work" / "llm-only",
        code_sha=args.code_sha,
    )
    records = write_llm_only_mechanism_execution(
        execution,
        args.outdir / "records",
    )
